...
TEST(Search_view_for_each, visits_subset_with_offset) {
  ...
  write_search_file(
      path,
      {{"/a", "http://example.com/a", "A Title", "A Desc", 80, 1, 2, 100},
       {"/b", "http://example.com/b", "B Title", "B Desc", 80, 3, 4, 100},
       {"/c", "http://example.com/c", "C Title", "C Desc", 80, 5, 6, 100},
       {"/d", "http://example.com/d", "D Title", "D Desc", 80, 7, 8, 100}});
  sourcemeta::one::SearchView view{path};
...
TEST(Search_view_for_each, visit_order_matches_at) {
  ...
  write_search_file(
      path, {{"/zebra", "http://example.com/zebra", "", "", 80, 11, 22, 100},
             {"/apple", "http://example.com/apple", "", "", 80, 33, 44, 100},
             {"/mango", "http://example.com/mango", "", "", 80, 55, 66, 100}});
  sourcemeta::one::SearchView view{path};
...
TEST(Search_view_for_each, malformed_record_field_lengths_stops) {
  ...
  auto payload{sourcemeta::one::make_search(
      {{"/foo", "http://example.com/foo", "Title", "Desc", 80, 1, 2, 100}})};
  sourcemeta::one::SearchIndexHeader header{};
