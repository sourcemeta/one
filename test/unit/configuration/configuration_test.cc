...
static auto authentication_with_path(const std::string &path)
    -> sourcemeta::core::JSON {
...
TEST(Configuration, authentication_rejects_path_with_extension) {
  EXPECT_THROW(
      sourcemeta::one::Configuration::parse(
          authentication_with_path("/internal/secret.json"), "/tmp/one.json",
          "."),
      sourcemeta::one::ConfigurationNonCanonicalAuthenticationPathError);
}

TEST(Configuration, authentication_rejects_path_with_percent_encoding) {
  EXPECT_THROW(
      sourcemeta::one::Configuration::parse(
          authentication_with_path("/internal/%73ecret"), "/tmp/one.json", "."),
      sourcemeta::one::ConfigurationNonCanonicalAuthenticationPathError);
}

TEST(Configuration, authentication_rejects_path_with_dot_segment) {
  EXPECT_THROW(
      sourcemeta::one::Configuration::parse(
          authentication_with_path("/internal/./secret"), "/tmp/one.json", "."),
      sourcemeta::one::ConfigurationNonCanonicalAuthenticationPathError);
}

TEST(Configuration, authentication_rejects_path_with_parent_segment) {
  EXPECT_THROW(
      sourcemeta::one::Configuration::parse(
          authentication_with_path("/public/../internal"), "/tmp/one.json",
          "."),
      sourcemeta::one::ConfigurationNonCanonicalAuthenticationPathError);
}

TEST(Configuration, authentication_rejects_uppercase_path) {
  EXPECT_THROW(
      sourcemeta::one::Configuration::parse(
          authentication_with_path("/Internal"), "/tmp/one.json", "."),
      sourcemeta::one::ConfigurationNonCanonicalAuthenticationPathError);
}

TEST(Configuration, authentication_accepts_a_canonical_path) {
  EXPECT_NO_THROW(sourcemeta::one::Configuration::parse(
      authentication_with_path("/internal/secret"), "/tmp/one.json", "."));
}

TEST(Configuration, authentication_accepts_a_trailing_slash_path) {
  EXPECT_NO_THROW(sourcemeta::one::Configuration::parse(
      authentication_with_path("/internal/"), "/tmp/one.json", "."));
}

TEST(Configuration, authentication_accepts_a_repeated_slash_path) {
  EXPECT_NO_THROW(sourcemeta::one::Configuration::parse(
      authentication_with_path("/internal//secret"), "/tmp/one.json", "."));
}

TEST(Configuration, authentication_accepts_the_root_under_a_base_path) {
  const auto raw_configuration{sourcemeta::core::parse_json(R"JSON({
  ...
  EXPECT_NO_THROW(sourcemeta::one::Configuration::parse(raw_configuration,
                                                        "/tmp/one.json", "."));
}
