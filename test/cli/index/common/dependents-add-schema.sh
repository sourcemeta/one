...
# When adding a new schema that references an existing one, only the
# dependents of the referenced schema and the new schema itself should
# rebuild. Unrelated schemas' dependents must not be scheduled.
...
# A's dependents must rebuild (D now references A)
grep -q "schemas/example/schemas/a/%/dependents.metapack" \
    "$TMP/output_producing.txt"

# D's dependents must rebuild (D is new)
