...
grep -q "explorer/example/schemas/a/%/dependents.metapack" \
    "$TMP/output_producing.txt"

# D's dependents must rebuild (D is new)
grep -q "explorer/example/schemas/d/%/dependents.metapack" \
    "$TMP/output_producing.txt"

# B and C's dependents must NOT rebuild (unrelated to D)
grep -q "explorer/example/schemas/b/%/dependents.metapack" \
    "$TMP/output_producing.txt" && exit 1
grep -q "explorer/example/schemas/c/%/dependents.metapack" \
    "$TMP/output_producing.txt" && exit 1
...
./explorer/example/schemas/d/%
./explorer/example/schemas/d/%/dependents.metapack
