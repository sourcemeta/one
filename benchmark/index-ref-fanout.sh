...
cat << 'EOF' > "$TMP/schemas/target.json"
{
...
echo "Filling registry to ${COUNT} schemas (\$ref fan-out)..." >&2
index=0
...
echo "Measuring: index ${COUNT} schemas (\$ref fan-out)..." >&2
START="$(nanoseconds)"
...
[
  {
    "name": "Index ${COUNT} schemas (\$ref fan-out)",
    "unit": "ms",
    "value": ${RESULT}
  }
]
