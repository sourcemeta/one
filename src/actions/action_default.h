  ...
  run(const std::filesystem::path &base, const std::span<std::string_view>,
    ...
    if (request.path() == "/") {
      if (request.prefers_html()) {
        ActionServeMetapackFile::serve(
            base / "explorer" / "%" / "directory-html.metapack",
    ...
    if (request.path().ends_with(".json")) {
      ActionJSONSchemaServe::serve(
          base, request.path().substr(1, request.path().size() - 6), request,
          response);
      return;
    }
    ...
    if (request.method() == "get" || request.method() == "head") {
      if (request.prefers_html()) {
        auto explorer_path{base / "explorer" / url_path / "%"};
        if (std::filesystem::exists(explorer_path / "schema-html.metapack") &&
          ...
          if (std::filesystem::exists(explorer_path)) {
            ...
            ActionServeMetapackFile::serve(base / "explorer" / "%" /
                                               "404.metapack",
        ...
        ActionJSONSchemaServe::serve(base, url_path, request, response);
      }
    } else {
      sourcemeta::one::json_error(
          request, response, sourcemeta::one::STATUS_NOT_FOUND, "not-found",
