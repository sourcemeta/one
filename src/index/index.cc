...
static auto DISPATCH(const std::filesystem::path &destination,
                     ...
                     sourcemeta::one::Build &adapter) -> bool {
...
static auto index_main(const std::string_view &program,
  ...
  const auto mark_configuration_path{adapter.path() / "configuration.json"};
  // Note we only write back if the content changed in order to not accidentally
  // bump up the file modified time
  adapter.output_write_json_if_different(mark_configuration_path,
                                         raw_configuration);
  ...
  const auto comment_path{adapter.path() / "comment.json"};
  ...
  const auto schemas_path{adapter.path() / "schemas"};
  const auto display_schemas_path{
      std::filesystem::relative(schemas_path, adapter.path())};
  sourcemeta::core::parallel_for_each(
      resolver.begin(), resolver.end(),
      [&schemas_path, &resolver, &mutex, &adapter, &mark_configuration_path,
  ...
  const auto explorer_path{adapter.path() / "explorer"};
  const auto display_explorer_path{
      std::filesystem::relative(explorer_path, adapter.path())};
  sourcemeta::core::parallel_for_each(
      resolver.begin(), resolver.end(),
      [&schemas_path, &explorer_path, &resolver, &mutex, &adapter,
       &mark_configuration_path, &mark_version_path](
        ...
        DISPATCH<sourcemeta::one::GENERATE_DEPENDENCIES>(
            ...
            {sourcemeta::one::Build::dependency(base_path / "schema.metapack"),
             sourcemeta::one::Build::dependency(mark_version_path)},
            resolver, mutex, "Analysing", schema.first, "dependencies",
            adapter);
        ...
        DISPATCH<sourcemeta::one::GENERATE_HEALTH>(
            ...
            mutex, "Analysing", schema.first, "health", adapter);
        ...
        if (attribute_not_disabled(schema.second.collection.get(),
          ...
          DISPATCH<sourcemeta::one::GENERATE_BLAZE_TEMPLATE>(
              ...
              schema.first, "blaze-exhaustive", adapter);
          DISPATCH<sourcemeta::one::GENERATE_BLAZE_TEMPLATE>(
              ...
              schema.first, "blaze-fast", adapter);
        }
        ...
        DISPATCH<sourcemeta::one::GENERATE_EXPLORER_SCHEMA_METADATA>(
            ...
            mutex, "Analysing", schema.first, "metadata", adapter);
      },
  ...
  if (std::filesystem::exists(schemas_path)) {
    for (const auto &entry :
      ...
      if (adapter.is_untracked_file(entry.path())) {
        continue;
      }
  ...
  const auto dependency_tree_path{adapter.path() / "dependency-tree.metapack"};
  ...
  const auto dependency_tree_rebuilt{
      DISPATCH<sourcemeta::one::GENERATE_DEPENDENCY_TREE>(
          ...
          display_schemas_path.string(), "dependencies", adapter)};
  ...
  sourcemeta::core::parallel_for_each(
      ...
      [&rework_entries, &mutex, &adapter, &dependency_tree_path](
  ...
  DISPATCH<sourcemeta::one::GENERATE_EXPLORER_SEARCH_INDEX>(
      ...
      "Producing", display_explorer_path.string(), "search", adapter);
  ...
  const auto routes_path{adapter.path() / "routes.bin"};
  const auto display_routes_path{
      std::filesystem::relative(routes_path, adapter.path())};
  ...
  if (app.contains("profile")) {
    ...
    std::vector<std::pair<std::filesystem::path, std::chrono::milliseconds>>
        durations;
    for (const auto &entry :
         std::filesystem::recursive_directory_iterator{adapter.path()}) {
