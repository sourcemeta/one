...
auto ActionMCP_v1::on_tools_call(const sourcemeta::core::JSON &request_json,
  ...
  try {
    return instance->mcp(id, *arguments, envelope);
  } catch (const std::exception &error) {
    return sourcemeta::one::mcp_make_tool_error(id, error.what());
  }
}

auto ActionMCP_v1::on_message(sourcemeta::one::HTTPRequest &request,
  ...
  if (!sourcemeta::one::mcp_is_request_method(method)) {
    ...
    envelope = this->on_tools_call(request_json, body);
