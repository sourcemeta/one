...
| `/audience`     | String  | :red_circle: **Yes** | N/A | The audience this instance identifies as. A token is accepted when its `aud` claim includes this value, so a token minted for several audiences at once is accepted as long as this one is among them. The [Model Context Protocol endpoint](api.md#model-context-protocol) is the exception: the specification has an MCP server accept only tokens issued for the endpoint itself, so a token naming the registry as a whole reaches everything this policy governs except that endpoint |
