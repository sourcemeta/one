...
| Property        | Type | Required | Default | Description |
|-----------------|------|----------|---------|-------------|
| `/issuer`       | String  | :red_circle: **Yes** | N/A | The token issuer to trust, matched against the `iss` claim. When `jwksUri` is omitted the key set is discovered from this issuer, so it must then be an `https` URL, and indexing refuses anything else. A trailing slash is accepted and dropped, since [OpenID Connect Discovery 1.0](https://openid.net/specs/openid-connect-discovery-1_0.html) removes it before appending the well-known suffix and requires the document to declare the prefix that was actually used, so it is not part of the identifier |
...
| `/issuer`       | String  | :red_circle: **Yes** | N/A | The OpenID Connect issuer to trust, matched against the identity token's `iss` claim and used to discover the provider's metadata, including the signing key set that verifies tokens. It must be an `https` URL, as OpenID Connect Discovery requires, and indexing refuses anything else. A trailing slash is accepted and dropped, since the specification removes it before appending the well-known suffix and requires the document to declare the prefix that was actually used, so it is not part of the identifier. Front a provider that only speaks plain HTTP with TLS termination and trust its certificate authority |
...
!!! warning
    ...
    one, and indexing refuses the configuration otherwise. A session cookie is
    ...
    loopback host such as `http://localhost:8000` is exempt, because a browser
    already treats it as a trustworthy origin.
